...
| Layer | Technology |
|-------|-----------|
| **Frontend** | React 18 + TypeScript + Vite SPA, Tailwind CSS v4, 42+ components, 5 contexts |
...
| **Desktop BI Command Center** | 1024px+ layout: collapsible sidebar (220px/64px), TopBar, stat cards, sortable MembersTable, Recharts trend + bar charts, member drill-down panel. All chart code lazy-loaded; main bundle ~257KB gzip. |
| **Compact Mobile Hero** | Condensed header for inner pages (training, readiness, gear, etc.) with countdown pill, crew name, and tab nav — maximizes content area on mobile. |
| **Member Drill-Down Panel** | 50/50 desktop split on Readiness view: clickable member table (left) + detail panel (right) with 52px readiness score, 4-category status bars, ⚠ alerts, and badge row. |

---

## Design System

TrailLog ships a full 8-phase design system (DESIGN_BIBLE.md):
