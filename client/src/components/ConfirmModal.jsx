...
export default function ConfirmModal({ title, message, confirmLabel, onConfirm, onCancel, danger = true, memberName }) {
  const { theme } = useTheme();

  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 999 }}
      onClick={onCancel}>
      <div onClick={e => e.stopPropagation()} style={{ background: theme.bgCard, borderRadius: 12, padding: 24, border: `1px solid ${danger ? theme.danger : theme.accent}40`, width: 300, textAlign: "center", boxShadow: "0 8px 32px rgba(0,0,0,0.3)" }}>
