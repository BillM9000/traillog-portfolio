...
export default function ProfilePage({ memberships, onBack, onEnterTroop, onLogout }) {
  ...
  const [leavingTroop, setLeavingTroop] = useState(null);
  ...
  const handleLeave = async (troopId, troopName) => {
  ...
  const handleWithdraw = async (troopId, troopName) => {
    setLeavingTroop(troopId);
    try {
      await api.leaveTroop(troopId);
      await refresh();
  ...
  const valueStyle = { fontSize: 14, color: theme.text, fontFamily: fontBody };
          ...
          <div key={m.troop_id} style={{
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: 11, color: theme.textDim, fontFamily: fontBody }}>
                {m.troop_council}{m.troop_location ? ` · ${m.troop_location}` : ""}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 8 }}>
            ...
            <div>
              ...
              <input type="password" value={confirmPw} onChange={e => setConfirmPw(e.target.value)}
                onKeyDown={e => e.key === "Enter" && changePassword()} style={inputStyle} />
            </div>
