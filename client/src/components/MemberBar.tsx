      ...
      <div key={m.is_manual ? `m-${m.id}` : `u-${m.user_id}`} onClick={() => !allCrewsMode && setActive(active === memberIdx ? null : memberIdx)} style={{
        ...
        <div style={{ position: "relative", flexShrink: 0 }}>
          {(m as AdventureMember & { avatar_url?: string }).avatar_url ? (
            <img src={(m as AdventureMember & { avatar_url?: string }).avatar_url} alt="" style={{ width: 40, height: 40, borderRadius: 20 }} />
          ) : (
            <div style={{
      ...
      {(() => {
        ...
        return (
          <div style={{ padding: "10px 14px", marginTop: 10, background: theme.name === "dark" ? "#1e2418" : "#f4f9ee", borderRadius: 10, border: `1px solid ${theme.borderAccent}` }}>
            ...
            <select onChange={(e: React.ChangeEvent<HTMLSelectElement>) => { if (e.target.value && onRequestLink) { onRequestLink(parseInt(e.target.value)); e.target.value = ""; } }} defaultValue="" style={{
