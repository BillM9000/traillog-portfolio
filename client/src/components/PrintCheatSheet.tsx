...
export default function PrintCheatSheet({ adventure, itinerary, onClose }: Props): React.JSX.Element | null {
  ...
  const print = (type: "pocket" | "full"): void => {
    ...
    const title = `${adventure?.name || "Trek"} — ${type === "pocket" ? "Pocket Card" : "Full Summary"}`;
    w.document.write(`<!DOCTYPE html><html><head><title>${title}</title>
      <style>
        body{font-family:Arial,sans-serif;padding:20px;max-width:750px;margin:0 auto}
        .print-toolbar{position:sticky;top:0;z-index:10;background:#3A4D2A;color:#fff;padding:10px 20px;margin:-20px -20px 20px -20px;display:flex;align-items:center;justify-content:space-between;font-family:Arial,sans-serif}
        ...
        @media print{body{padding:10px}.no-print,.print-toolbar{display:none}}
      </style>
    </head><body>
      ...
      ${type === "pocket" ? pocketHTML : fullHTML}
    </body></html>`);
    w.document.close();
  };
