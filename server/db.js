...
function seedGearCatalog() {
  ...
  // Full catalog: 76 items across 12 categories with 4 sharing types
  // Categories: Pack & Carry, Shelter, Sleep System, Clothing, Footwear,
  //   Navigation, Hydration & Water, Food & Cooking, Fire & Light,
  //   First Aid & Safety, Hygiene & LNT, Sun & Weather, Repair & Tools, Communication
  //
  // Each item: [name, category, subcategory, weight_oz, priority, is_crew_shared, description]
  // Priority levels: essential, recommended, optional
  // Sharing types: personal (your own), crew (shared among crew), buddy (split with tent partner), provided (supplied on-site)
  //
  // [Full 76-item catalog redacted — proprietary domain knowledge]
  // Below are 5 representative samples showing the data structure:

  const S = [
    ["Backpacking Pack (65-75L)", "Pack & Carry", "Backpack", 52, "essential", 0, "Torso-fit suspension; adjustable hip belt"],
    ["Sleeping Bag (20F rated)", "Sleep System", "Sleeping Bag", 40, "essential", 0, "Temperature-rated for alpine conditions"],
    ["Water Filter / Purifier", "Hydration & Water", "Filtration", 3, "essential", 1, "All water must be treated"],
    ["Backpacking Tent (2-3 person)", "Shelter", "Tent", 56, "essential", 1, "Freestanding preferred for rocky sites"],
    ["Headlamp (primary)", "Fire & Light", "Headlamp", 3, "essential", 0, "200+ lumens; red mode for night vision"],
    // ... 71 more items in the full catalog
  ];

  // Sharing type overrides determine how each item's weight is calculated:
  //   personal → counted in individual pack weight
  //   crew → split among crew, excluded from individual weight calc
  //   buddy → split between tent partners, excluded from individual weight calc
  //   provided → supplied on-site, excluded from weight calc
  const sharingOverrides = {
    "Backpacking Tent (2-3 person)": "buddy",
    "Water Filter / Purifier": "crew",
    // ... more overrides in the full catalog
  };
