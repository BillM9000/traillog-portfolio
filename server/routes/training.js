...
router.get("/api/adventures/:adventureId/training-events", requireAuth, requireAdventureMember, async (req, res) => {
  try { res.json(await getTrainingEvents(parseId(req.params.adventureId))); }
  catch (e) { safeError(res, e); }
});

// Export training events as .ics calendar file
router.get("/api/adventures/:adventureId/training-events/export.ics", requireAuth, requireAdventureMember, async (req, res) => {
  try {
    const advId = parseId(req.params.adventureId);
    const events = (await getTrainingEvents(advId)).filter(e => e.status === "active" && e.type === "scheduled");
    const adventure = await getAdventure(advId);
    const troop = adventure ? await getTroop(adventure.troop_id) : null;
...
router.post("/api/adventures/:adventureId/training-events", requireAuth, requireAdventureAdmin, validate(createTrainingEventSchema), async (req, res) => {
  try {
    const advId = parseId(req.params.adventureId);
    ...
    const event = await createTrainingEvent(advId, { date, period: period || "all", time_label, location, notes, type: type || "proposed" }, req.user.id);

    // Only email members when event is scheduled (not proposed)
    if (event.type === "scheduled") {
      const adventure = await getAdventure(advId);
      const troopForEmail = adventure ? await getTroop(adventure.troop_id) : null;
      const members = (await getAdventureMembers(advId)).filter(m => !m.is_manual);
...
router.delete("/api/adventures/:adventureId/training-events/:eventId", requireAuth, requireAdventureAdmin, async (req, res) => {
  try {
    await deleteTrainingEvent(parseId(req.params.eventId));
    res.json({ ok: true });
  } catch (e) { safeError(res, e); }
});

router.put("/api/adventures/:adventureId/training-events/:eventId/rsvp", requireAuth, requireAdventureMember, validate(rsvpSchema), async (req, res) => {
  try {
    const { status } = req.body;
    if (!["going", "cant"].includes(status)) return res.status(400).json({ error: "status must be going or cant" });
    await upsertTrainingRsvp(parseId(req.params.eventId), req.user.id, status);
    res.json({ ok: true });
  } catch (e) { safeError(res, e); }
});

// Update event type/status (propose → schedule → complete → cancel)
router.put("/api/adventures/:adventureId/training-events/:eventId/status", requireAuth, requireAdventureAdmin, async (req, res) => {
  try {
    ...
    await updateTrainingEventStatus(eventId, newType, newStatus);
    ...
    if (newStatus === "completed") {
      await syncAttendanceSkills(advId);
    }
...
router.put("/api/adventures/:adventureId/training-events/:eventId", requireAuth, requireAdventureAdmin, validate(updateTrainingEventSchema), async (req, res) => {
  try {
    const eventId = parseId(req.params.eventId);
    const event = await getTrainingEvent(eventId);
    if (!event) return res.status(404).json({ error: "event not found" });
    ...
    await updateTrainingEvent(eventId, { date, period, time_label, location, notes });
    res.json({ ok: true });
  } catch (e) { safeError(res, e); }
});

// Bulk mark attendance for a completed event
router.post("/api/adventures/:adventureId/training-events/:eventId/attendance", requireAuth, requireAdventureAdmin, async (req, res) => {
  try {
    const eventId = parseId(req.params.eventId);
    const advId = parseId(req.params.adventureId);
    ...
    await bulkMarkAttendance(eventId, attendees.map(id => parseId(id)), req.user.id);
...
router.put("/api/adventures/:adventureId/training-events/:eventId/attendance/self", requireAuth, requireAdventureMember, async (req, res) => {
  try {
    const eventId = parseId(req.params.eventId);
    const advId = parseId(req.params.adventureId);
    ...
    await markAttendance(eventId, req.user.id, attended ? 1 : 0, req.user.id);
    await syncAttendanceSkills(advId);
    res.json({ ok: true });
  } catch (e) { safeError(res, e); }
});
...
router.get("/api/adventures/:adventureId/training-events/:eventId/attendance", requireAuth, requireAdventureMember, async (req, res) => {
  try {
    res.json(await getEventAttendance(parseId(req.params.eventId)));
  } catch (e) { safeError(res, e); }
});

// Get member attendance count for readiness
router.get("/api/adventures/:adventureId/members/:userId/attendance-count", requireAuth, requireAdventureMember, async (req, res) => {
  try {
    const count = await getMemberAttendanceCount(parseId(req.params.adventureId), parseId(req.params.userId));
    res.json({ count });
  } catch (e) { safeError(res, e); }
});
