...
TrailLog uses **PostgreSQL** as its sole data store, accessed through the **pg (node-postgres)** driver with a connection pool in asynchronous mode. The database runs on the VPS host and the Docker container connects via `the VPS host via connection pool`. There is no ORM; all database access uses hand-written SQL with parameterized queries. The canonical schema is defined in `db/schema.pg.sql`.
