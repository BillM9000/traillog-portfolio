...
trek preparation, built with Express.js, SQLite, and React 18.
...
|-------|---------------|----------|
| User PII (email, name, parent_email, parent_email_2) | Sensitive | SQLite database |
| Password hashes (bcrypt, 10 rounds) | Sensitive | SQLite database |
| Google OAuth IDs | Sensitive | SQLite database |
| Session tokens | Sensitive | SQLite sessions table |
| Verification tokens (email confirmation) | Sensitive | SQLite database |
| Invitation tokens (troop/adventure invites) | Sensitive | SQLite database |
| Troop and adventure data (names, dates, readiness) | Internal | SQLite database |
| Gear selections and pack weight data | Internal | SQLite database |
...
- SQLite database file on Docker named volume

## Trust Boundaries

```
  Browser              Traefik              Docker Container         SQLite
    |                    |                       |                     |
    |--- HTTPS/TLS ----->|--- localhost:3614 --->|--- filesystem I/O ->|
    |                    |                       |                     |
...
3. **Container to SQLite** -- Filesystem access within the container. The database
   file lives on a Docker named volume. The container process runs as non-root
...
| Database file exposure | Low | SQLite file is on a Docker named volume, not bind-mounted to host filesystem. Container runs as non-root user. Docker socket not exposed. | Mitigated |
...
| Database locking | Low | SQLite WAL mode allows concurrent reads. Write transactions are short (deleteAdventure wraps 8 table deletions in a single transaction). | Mitigated |
...
**Risk:** If the VPS is compromised, the SQLite database is readable in plaintext.

**Current mitigation:** Docker container isolation, non-root process user, VPS
access restricted to SSH key authentication, .env file permissions set to 600.

**Recommendation:** Consider host-level disk encryption (LUKS) on the VPS. SQLCipher
is a possibility but adds native binding complexity to the Docker build.
...
**Current mitigation:** Rolling 10 daily backups via SQLite `.backup` command.
